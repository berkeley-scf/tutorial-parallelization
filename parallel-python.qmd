...
Next let's consider JIT compiling it, which should [fuse the vectorized operations](../parallel-julia#loops-and-fused-operations) and avoid temporary objects. The JAX docs have a [nice discussion](https://jax.readthedocs.io/en/latest/notebooks/thinking_in_jax.html#to-jit-or-not-to-jit) of when JIT compilation will be beneficial.
...
Linear algebra in JAX will use multiple threads ([as discussed for numpy](./#2-threading)).
